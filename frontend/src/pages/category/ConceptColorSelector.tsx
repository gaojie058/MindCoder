...
export default function ColorSelector({ conceptId, onColorChange }: Props) {
  const { conceptData, setConceptData } = useConceptStore();
  const colorFromStore =
    conceptData.find((concept) => concept.id === conceptId)?.color || "#E3C8C0";
  ...
  const updateColor = (newColor: string) => {
    const updatedData = conceptData.map((c) =>
      c.id === conceptId ? { ...c, color: newColor } : c
    );
